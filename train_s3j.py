...
import time
import torch
from lib.coverage_weighted_gradient_protection_refinement import CoverageWeightedGradientProtectionWrapper, set_s3j_training_target
...
def seed_everything(seed=2401):
    seed = int(__import__("os").environ.get("FIANET_SEED", seed))
    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            if bert_model is not None:
                ...
                if model.training:
                    set_s3j_training_target(model, target)
...
def main(args):
    ...
    # resume training
    # ========================================================
    # S3-J: wrap the existing S2 high-resolution residual head
    ...
    if getattr(
        ...
        raise RuntimeError(
            "S3-J requires --use-small-refine, but "
            "model.small_refinement_head is None."
        )

    _s3j_base_head = model.small_refinement_head

    try:
        _s3j_device = next(
            _s3j_base_head.parameters()
        ).device
    ...
    print(
        "S3-J bounded gated residual wrapper enabled: "
    ...
    if getattr(
        ...
        print(
            "S3-J optimizer added refinement parameters: "
            "{}".format(
                sum(
                    parameter.numel()
                    for parameter
