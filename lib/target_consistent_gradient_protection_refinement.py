...
class TargetConsistentGradientProtectionWrapper(
    BoundedGatedResidualWrapper
):
    """
    S3-I: Target-Consistent Gradient Protection.
    ...
    def __init__(
        ...
        **kwargs
        ...
        print(
            "S3-I target-consistent gradient protection enabled: "
            "protect_strength={:.4f}, "
    ...
    def _build_local_gt_foreground(
        ...
        # Tiny-object-safe downsampling.
        #
        # Max pooling marks a local cell as foreground if
        # ANY source GT foreground pixel falls into it.
        # This avoids losing tiny GT targets with nearest
        # downsampling.
        if (
            in_h >= out_h
            and in_w >= out_w
    ...
    def forward(
        ...
        if self._training_target is None:
            raise RuntimeError(
                "S3-I training forward has no GT target. "
                "Call set_s3i_training_target(model, target) "
        ...
        if (
            ...
            raise RuntimeError(
                "S3-I requires two-class main logits, "
                "got {}".format(
...
def set_s3i_training_target(
    ...
    if not hasattr(
        ...
        raise RuntimeError(
            ...
            "S3-I target injection; got {}".format(
                type(head).__name__
            )
        )
