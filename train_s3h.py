...
import time
import torch
from lib.budgeted_weak_target_rescue_refinement import BudgetedWeakTargetRescueWrapper
...
def main(args):
    ...
    # resume training
    # ========================================================
    # S3-H: wrap the existing S2 high-resolution residual head
    ...
    if getattr(
        ...
        raise RuntimeError(
            "S3-H requires --use-small-refine, but "
            "model.small_refinement_head is None."
        )

    _s3h_base_head = model.small_refinement_head

    try:
        _s3h_device = next(
            _s3h_base_head.parameters()
        ).device
    ...
    print(
        "S3-H bounded gated residual wrapper enabled: "
    ...
    if getattr(
        ...
        print(
            "S3-H optimizer added refinement parameters: "
            "{}".format(
                sum(
                    parameter.numel()
                    for parameter
