...
import time
import torch
from lib.weak_target_flip_rescue_refinement import WeakTargetFlipRescueWrapper
...
def main(args):
    ...
    # resume training
    # ========================================================
    # S3-F: wrap the existing S2 high-resolution residual head
    ...
    if getattr(
        ...
        raise RuntimeError(
            "S3-F requires --use-small-refine, but "
    ...
    print(
        "S3-F bounded gated residual wrapper enabled: "
    ...
    if getattr(
        ...
        print(
            "S3-F optimizer added refinement parameters: "
            "{}".format(
                sum(
                    parameter.numel()
                    for parameter
