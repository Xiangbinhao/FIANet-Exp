import sys
import runpy
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parent
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from lib import segmentation
from lib.weak_evidence_conditional_refinement import (
    WeakEvidenceConditionalResidualWrapper,
)


_original_lavt_one = segmentation.lavt_one


def _s3d_lavt_one(*args, **kwargs):
    model = _original_lavt_one(*args, **kwargs)

    base_head = getattr(
        model,
        "small_refinement_head",
        None,
    )

    if base_head is None:
        raise RuntimeError(
            "S3-D evaluation requires --use-small-refine."
        )

    model.small_refinement_head = (
        WeakEvidenceConditionalResidualWrapper(
            base_head=base_head,
            alpha_init=0.10,
            gate_init=0.20,
            relax_init=0.75,
            ambiguity_center=0.20,
            ambiguity_temperature=0.05,
        )
    )

    print(
        "S3-D evaluation wrapper enabled: "
        "alpha_init=0.10, gate_init=0.20, "
        "relax_init=0.75, ambiguity_center=0.20, "
        "ambiguity_temperature=0.05"
    )

    return model


segmentation.lavt_one = _s3d_lavt_one

runpy.run_path(
    str(PROJECT_ROOT / "test.py"),
    run_name="__main__",
)
