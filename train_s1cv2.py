...
from loss.foreground_size_aux_loss_v2 import ScheduledForegroundSizeAuxiliaryLoss
...
def main(args):
    ...
    # S1-Cv2 modifies training loss only.
    # Validation remains identical to E0.
    train_criterion = criterion

    if getattr(
        args,
        'use_scheduled_foreground_size_aux_loss',
        False,
    ):
        train_criterion = (
        ...
        if configured_total != args.epochs:
            raise ValueError(
                "S1-Cv2 schedule totals {} epochs, "
                "but --epochs is {}.".format(
                    configured_total,
                    args.epochs,
                )
            )
        ...
        print("Sampler: original RandomSampler")
        print("Model: original E0")
        print(
            "=================================\n"
        )
    ...
    training_start_epoch = max(
        0,
        resume_epoch + 1,
    )

    training_end_epoch = args.epochs
    ...
    if getattr(
        ...
        training_end_epoch = min(
            args.epochs,
            training_start_epoch
            + args.s1cv2_pilot_epochs,
        )

        print(
            "S1-Cv2 pilot mode: epochs {}-{}; "
            "LR scheduler still uses {} epochs."
            .format(
                training_start_epoch,
                training_end_epoch - 1,
                args.epochs,
            )
        )
    ...
    for epoch in range(
        ...
        train_one_epoch(model, train_criterion, optimizer, data_loader, lr_scheduler, epoch, args.print_freq, iterations, bert_model, scaler)
