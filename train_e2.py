    ...
    for i, data in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if isinstance(output, (tuple, list)):
                final_output, coarse_output = output

                # 将像素级 GT 转换为低分辨率定位标签。
                # 只要某个粗网格包含至少一个前景像素，
                # 该网格就被视为目标区域。
                coarse_target = F.adaptive_max_pool2d(
                    target.unsqueeze(1).float(),
                    output_size=coarse_output.shape[-2:],
                )
                coarse_target = coarse_target.squeeze(1).long()
                ...
                loss = (
                    final_loss
                    + args.aux_loc_weight * coarse_loss
                )
        ...
        final_loss_value = final_loss.detach().item()

        if coarse_loss is not None:
            coarse_loss_value = coarse_loss.detach().item()
        else:
            coarse_loss_value = 0.0

        metric_logger.update(
            final_loss=final_loss_value,
            coarse_loss=coarse_loss_value,
        )
...
def main(args):
    ...
    # optimizer
    # E1 coarse localization head is not part of the
    # original manually specified optimizer groups.
    if getattr(model, 'coarse_head', None) is not None:
        coarse_parameters = [
            parameter
            for parameter in model.coarse_head.parameters()
            if parameter.requires_grad
        ]

        params_to_optimize.append({
            "params": coarse_parameters,
        })

        print(
            "E1 optimizer added coarse-head parameters: {}".format(
                sum(
                    parameter.numel()
                    for parameter in coarse_parameters
                )
            )
        )

    # E2 residual refinement head parameters.
    if getattr(model, 'refinement_head', None) is not None:
        ...
        params_to_optimize.append({
            "params": refinement_parameters,
        })

        print(
            "E2 optimizer added refinement parameters: {}".format(
                sum(
                    parameter.numel()
                    for parameter in refinement_parameters
                )
            )
        )
    ...
    for epoch in range(max(0, resume_epoch+1), args.epochs):
        ...
        train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, epoch, args.print_freq, iterations, bert_model, scaler)
