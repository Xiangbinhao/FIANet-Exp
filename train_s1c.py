...
from loss.foreground_size_aux_loss import ForegroundSizeAuxiliaryLoss
...
def main(args):
    ...
    # S1-C modifies only the training loss.
    # Validation remains identical to E0.
    train_criterion = criterion

    if getattr(
        args,
        'use_foreground_size_aux_loss',
        False,
    ):
        train_criterion = (
        ...
        print(
            "Tiny lambda: {:.3f}".format(
                args.s1c_tiny_lambda
            )
        )

        print(
            "Small lambda: {:.3f}".format(
                args.s1c_small_lambda
            )
        )
