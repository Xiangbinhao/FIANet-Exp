...
from data.size_aware_sampler import build_size_aware_sampler
import wandb
import random
...
def main(args):
    ...
    if getattr(args, 'use_size_aware_sampler', False):
        train_sampler, s1a_sampler_summary = build_size_aware_sampler(
            dataset,
            args,
            verbose=True,
        )
    ...
    # optimizer
    # S2 + S1-A:
    # The S2 high-resolution refinement head is outside
    # the original manually defined optimizer groups.
    if getattr(
        model,
        ...
        if not small_refinement_parameters:
            raise RuntimeError(
                "S2 refinement head has no trainable parameters"
            )
        ...
        print(
            "S2_S1A optimizer added refinement parameters: "
            "{}".format(
                sum(
                    parameter.numel()
                    for parameter
