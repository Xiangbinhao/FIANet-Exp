...
import time
import torch
from lib.target_consistent_gradient_protection_refinement import TargetConsistentGradientProtectionWrapper, set_s3i_training_target
...
def main(args):
    ...
    # resume training
    # ========================================================
    # S3-I: wrap the existing S2 high-resolution residual head
    ...
    if getattr(
        ...
        raise RuntimeError(
            "S3-I requires --use-small-refine, but "
            "model.small_refinement_head is None."
        )

    _s3i_base_head = model.small_refinement_head

    try:
        _s3i_device = next(
            _s3i_base_head.parameters()
        ).device
    ...
    print(
        "S3-I bounded gated residual wrapper enabled: "
    ...
    if getattr(
        ...
        print(
            "S3-I optimizer added refinement parameters: "
            "{}".format(
                sum(
                    parameter.numel()
                    for parameter
