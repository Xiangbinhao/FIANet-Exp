...
class S3AResidualCapture(object):
    """
    Capture the raw output of the S2 refinement head.

    The hook changes neither the model output nor its checkpoint
    structure. The captured tensor keeps its gradient graph.
    """
...
def train_one_epoch(
    ...
    for i, data in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            # S2 normally returns final logits only.
            # Keep tuple compatibility for defensive checking.
            ...
            if small_residual_logits is None:
                raise RuntimeError(
                    "S3-A did not capture the output of "
                    "small_refinement_head. Confirm that "
                    "--use-small-refine is enabled."
                )

            # Match the interpolation used by the real S2
            ...
            if (
                ...
                raise RuntimeError(
                    "S3-A residual/final shape mismatch: "
                    "{} versus {}".format(
                        tuple(
                            small_residual_logits.shape
                        ),
            ...
            #     final = base + residual
            #
            # Reconstruct a detached base prediction, then add
            # the live residual back. The auxiliary loss therefore
            # sends gradients only through small_refinement_head.
            ...
            # The following official logic should continue to
            # operate on the normal final segmentation logits.
            output = final_output
