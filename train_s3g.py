...
import time
import torch
from lib.semantic_detail_consensus_rescue_refinement import SemanticDetailConsensusRescueWrapper
...
def main(args):
    ...
    # resume training
    # ========================================================
    # S3-G: wrap the existing S2 high-resolution residual head
    ...
    if getattr(
        ...
        raise RuntimeError(
            "S3-G requires --use-small-refine, but "
            "model.small_refinement_head is None."
        )

    _s3g_base_head = model.small_refinement_head

    try:
        _s3g_device = next(
            _s3g_base_head.parameters()
        ).device
    ...
    print(
        "S3-G bounded gated residual wrapper enabled: "
    ...
    if getattr(
        ...
        print(
            "S3-G optimizer added refinement parameters: "
            "{}".format(
                sum(
                    parameter.numel()
                    for parameter
